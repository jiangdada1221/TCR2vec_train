...
@registry.register_task_model('mlm_spm_full', 'transformer')
class ProteinBertForMaskedLM(ProteinBertAbstractModel):
    ...
    def __init__(self, config,weight=1.0):
        ...
        self.mlm = MLMHead(
            config.hidden_size, config.vocab_size, config.hidden_act, config.layer_norm_eps,
            ignore_index=-1)
        from .modeling_utils import PRHead
        self.prhead = PRHead('mean')
        # self.weight = config.spm_weight
        # print('weight of SPM: ',self.weight)
        self.init_weights()
        self.tie_weights()
    ...
    def forward(self,
                ...
                seqs_cdr3=None):        
        outputs = self.bert(input_ids, input_mask=input_mask)        
        outputs_ori = self.bert(ori_ids,input_mask=input_mask) #get the output of bert from unmask seq data
        sequence_output_ori = outputs_ori[0]

        sequence_output, pooled_output = outputs[:2]
        # add hidden states and attention if they are here
        outputs1 = self.mlm(sequence_output, targets) 
        outputs = outputs1 + outputs[2:]
        # (loss), prediction_scores, (hidden_states), (attentions)
        # print(outputs1)
        if seqs_cdr3 is not None and self.spm_weight != 0:
            out2 = self.prhead(sequence_output_ori, input_mask,seqs_cdr3=seqs_cdr3)
            outputs1[0][1].update({'PR':out2})
            out0_new = ((outputs1[0][0]+self.spm_weight*out2,outputs1[0][1]),outputs1[1])                
            out_new = out0_new + outputs[2:]
            return out_new
        else :
            return outputs

@registry.register_task_model('mlm_spm_cdr3', 'transformer')
class ProteinBertForMaskedLM(ProteinBertAbstractModel):
