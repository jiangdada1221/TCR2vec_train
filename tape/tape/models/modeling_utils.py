...
score = parasail.nw_stats
#score = parasail.sw_stats
# blosum62 = substitution_matrices.load("BLOSUM62")
p_matrix = getattr(parasail, 'blosum62')
...
class PredictionHeadTransformv2(nn.Module):
...
def compute_lv_sim(seqs,penalty=False,normalize=False):
    '''
    compute levenstein distance for seqs
    '''
    dis_x = np.zeros((len(seqs),len(seqs)))
    for i in range(len(seqs)):
        for j in range(i,len(seqs)):
            if penalty:
                sim = score(seqs[i],seqs[j],open=6,extend=1,matrix=p_matrix).score #larger score larger similarity
                if normalize:
                    sim = sim / float(max(len(seqs[i]),len(seqs[j])))
            else :
                sim = Levenshtein.ratio(seqs[i],seqs[j])
            dis_x[i,j] = sim
            dis_x[j,i] = sim
    return dis_x
class corLoss(nn.Module):
    ...
    def forward(self,dis_x,dis_y):
        cor = self.correlation(dis_x,dis_y)
        return cor
    def correlation(self,X,Y):
        ...
        # return torch.sum((X - torch.mean(X)) * (Y - torch.mean(Y)) ) / (torch.sqrt(torch.sum((X-torch.mean(X))**2) * torch.sum((Y-torch.mean(Y))**2)))
        return r
        #np.sum((X - np.mean(X)) * (Y - np.mean(Y)) ) / (np.sqrt(np.sum((X-np.mean(X))**2) * np.sum((Y-np.mean(Y))**2)))
...
class PRHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.loss = corLoss()
    def forward(self, hidden_states, mask,seqs_cdr3=None):
        assert seqs_cdr3 is not None        
        if self.pool_method == 'mean':            
            # hidden_states = torch.mean(hidden_states,1) #B x L x d -> B x d
            #filter out pad tokens
            bs,L = mask.shape
            emb_size = hidden_states.shape[-1]
            sum_0 = mask[0].sum().item()
            masks = mask.repeat(emb_size,1,1)        
            masks = masks.permute((1,2,0))        
            hidden_states = hidden_states * masks
            hidden_states = torch.sum(hidden_states,1)                
            hidden_states = hidden_states / torch.sum(mask,1).view(-1,1)
        #outputs = (hidden_states,)
        if seqs_cdr3 is not None:
            seqs = np.array(list(seqs_cdr3))
            # print(seqs[:3])
            dis_x = compute_lv_sim(seqs,True,True) ### normalize!            
            dis_x = dis_x.astype(np.float16)
            dis_x = torch.from_numpy(dis_x).to('cuda:0')
            dis_y = pdist(hidden_states)            
            # print(dis_y[:3,:3])
            loss_spm = self.loss(dis_x.flatten(),dis_y.flatten())    
            # print(loss_spm)
        return loss_spm
