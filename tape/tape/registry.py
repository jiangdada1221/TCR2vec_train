...
class Registry:
    ...
    def get_task_model(cls,
        ...
        if load_dir is not None:
            ...
            model = model_cls.from_pretrained('bert-base') #pretrain model if from checkpoint remember to change it.
            if config_file is not None:
                config_class = model_cls.config_class
                config = config_class.from_json_file(config_file)
            #config has spm_weight
            model.spm_weight = config.spm_weight
        else:            
            config_class = model_cls.config_class
            if config_file is not None:
                config = config_class.from_json_file(config_file)
