...
class TCRseq(Dataset):
    """Creates a dataset from an csv file.
    Args:
        data_file (Union[str, Path]): Path to lmdb file.
        in_memory (bool, optional): Whether to load the full dataset into memory.
            Default: False.
    ...
    def __init__(self,
                 ...
                 in_memory: bool = False):
        import copy
        #original_file = copy(data_file)
        data_file = Path(data_file)
        # if not data_file_.exists():
        #     data_file = Path(str(data_file).replace('.csv','_1.npy'))
        #     if not data_file.exists():
        #         data_file = Path(str(data_file).replace('_1.npy','.npy'))
        #         if not data_file.exists():
        #             raise FileNotFoundError(data_file)
        ...
        if not data_file.exists():
            ...
            print(data_file)
            exit()
            data_file = str(data_file).replace('.csv','.tsv')
        if str(data_file).endswith('.csv'):
            print('load csv file')
            self.data = pd.read_csv(data_file)['CDR3.beta'].values
        else :
            #self.data = pd.read_csv(data_file,compression='gzip')['CDR3.beta'].values
            # if str(data_file)
            # print('npy file')
            # #self.data = np.lib.format.open_memmap(data_file,mode='r')
            # if '1' not in str(data_file):
            #     self.data = np.lib.format.open_memmap(data_file,mode='r')
            # else :
            #     self.data = []
            #     for i in range(1,11):
            #         if i != 1:
            #             data_file = str(data_file).replace(f'{i-1}.npy',f'{i}.npy')
            #         print(f'processed data file: {data_file}')                
            #         self.data.append(np.lib.format.open_memmap(data_file,mode='r'))  
            print(data_file)
            if 'TCRseqs_train' in str(data_file):
                self.data = pd.read_csv(data_file,compression='gzip')['CDR3.beta'].values
            else :
                self.data = pd.read_csv(data_file,compression='gzip')['CDR3.beta'].values          
            #self.data = pd.read_csv(data_file,compression='gzip')['CDR3.beta'].values
...
class MaskedLanguageModelingDataset(Dataset):
    ...
    def __init__(self,
        ...
        #data_file = f'TCRseqs_{split}.csv'
        data_file = 'data_whole.csv'
        #data_file = 'TCRseqs_valid.csv'
        #now for the paired version
        self.data = dataset_factory(data_path / data_file, in_memory) #change it to paired version
...
#Yuepeng's added new task
@registry.register_task('mlm_spm_full')
class MaskedLanguageModelingDataset(Dataset):
    """Creates the Masked Language Modeling Pfam Dataset
    Args:
        data_path (Union[str, Path]): Path to tape data root.
    ...
    def __init__(self,
        ...
        data_path = Path(data_path)  
        data_file = f'Emerson_{split}.tsv'
        self.data = dataset_factory(data_path / data_file, in_memory) #change it to paired version
    ...
    def __getitem__(self, index):
        item_whole = self.data[index] #(cdr3,v,j,full)
        item,item_cdr3 = item_whole[-1],item_whole[-1] #item is full length seq, item_cdr3 is cdr3 sequence     
        tokens = self.tokenizer.tokenize(item)
        tokens = self.tokenizer.add_special_tokens(tokens)
        ...
        masked_token_ids = np.array(
            self.tokenizer.convert_tokens_to_ids(masked_tokens), np.int64)
        input_mask = np.ones_like(masked_token_ids) #check here

        return masked_token_ids,token_ids, input_mask, labels,item_cdr3

    def collate_fn(self, batch: List[Any]) -> Dict[str, torch.Tensor]:
        # input_ids, input_mask, lm_label_ids, clan, family = tuple(zip(*batch))
        input_ids,ori_ids, input_mask, lm_label_ids,seq= tuple(zip(*batch))

        input_ids = torch.from_numpy(pad_sequences(input_ids, 0))
        input_mask = torch.from_numpy(pad_sequences(input_mask, 0))

        ori_ids = torch.from_numpy(pad_sequences(ori_ids, 0))
...
@registry.register_task('mlm_spm_cdr3')
class MaskedLanguageModelingDataset(Dataset):
    """Creates the Masked Language Modeling Pfam Dataset
    Args:
        data_path (Union[str, Path]): Path to tape data root.
    ...
    def __getitem__(self, index):
        item_whole = self.data[index] #(cdr3,v,j,full)
        item,item_cdr3 = item_whole[0],item_whole[0] # only modification here item is full length seq, item_cdr3 is cdr3 sequence
        # tokens = self.tokenizer.tokenize(item['primary'])
        ##comment block:
        tokens = self.tokenizer.tokenize(item)
        tokens = self.tokenizer.add_special_tokens(tokens)
