...
class ForwardRunner:
    ...
    def forward(self,
        ...
        if self.device.type == 'cuda':
            seqs = batch['seqs_cdr3']
            batch = {name: tensor.cuda(device=self.device, non_blocking=True)
                     for name, tensor in batch.items() if type(tensor)!= tuple}
            batch['seqs_cdr3'] = seqs            
...
def run_train(model_type: str,
    ...
    viz.watch(model)
    logger.info('The weight of SPM: ' + str(model.spm_weight))
    logger.info(
        f"device: {device} "
        f"n_gpu: {n_gpu}, "
